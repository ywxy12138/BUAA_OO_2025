...
public class Elevator {
    ...
    public boolean canOpenForOut(int transferFloor, String side) {
        for (Person person : inPeople) {
            if (person.canOpenForOut(transferFloor, side)) {
                return true;
            }
        }
        return false;
    ...
    public double out(double virtualTime, boolean isUp, int targetFloor,
        boolean isAllOut, boolean isVirtual, boolean isSchedule, int transferFloor, String side) {
        int len = inPeople.size();
        ArrayList<Person> newInPeople = new ArrayList<>();
        double totalTime = 0;
        for (int i = 0; i < len; i++) {
            Person person = inPeople.get(i);
            if (isAllOut || (isSchedule && person.canOpenForScheduleOut(isUp, targetFloor))
                || (!isSchedule && person.canOpenForOut(transferFloor, side))) {
                if (person.getIfArrived()) {
                    if (!isVirtual) {
                        Output.printOutS(person.getPersonId(),
                            ChangeFloor.getFloor(currentFloor), person.getElevatorId());
                    }
                    else {
                        totalTime += person.getPriority() * virtualTime;
                    }
                }
                else {
                    if (!isVirtual) {
                        Output.printOutF(person.getPersonId(),
                            ChangeFloor.getFloor(currentFloor), person.getElevatorId());
    ...
    public void in(SubTable requestTable, boolean isUp, boolean isVirtual,
        ArrayList<Person> virtualPeople, int targetFloor,
        boolean isSchedule) {
            ...
            if (!inPeople.contains(person)) {
                if (!isVirtual) {
                    if (!person.isReceived()) {
                        Output.printReceive(person.getPersonId(), person.getElevatorId());
                        person.setIsReceived(true);
                    }
                    Output.printIn(person.getPersonId(),
                        ChangeFloor.getFloor(currentFloor), person.getElevatorId());
