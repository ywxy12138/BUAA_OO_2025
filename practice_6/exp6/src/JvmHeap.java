...
public class JvmHeap extends MyHeap<MyObject> {//包含题目[1]
    ...
    public void removeUnreferenced() {
       ...
       //[1]
        Object[] newElements = new Object[getSize() + 1];
        int len = 0;
        for (int i = 1; i <= getSize(); i++) {
            if (((MyObject)elements[i]).isReferenced()) {
                newElements[++len] = elements[i];
            }
        }
        for (int i = 1; i < len; i++) {
            for (int j = i + 1; j <= len; j++) {
                if (((MyObject)(newElements[j])).compareTo(((MyObject) newElements[i])) <= 0) {
                    MyObject temp = (MyObject) newElements[j];
