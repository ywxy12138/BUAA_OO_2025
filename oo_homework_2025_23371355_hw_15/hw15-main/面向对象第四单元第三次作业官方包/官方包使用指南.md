...
class Sample {
    public static void main(String[] args) {
        Map<LibraryBookIsbn, Integer> bookList = LibraryIO.SCANNER.getInventory();	// 获取图书馆内所有书籍ISBN号及相应副本数
        while (true) {
...
> 注意若输入的命令中描述书籍使用的是“类别号-序列号”（如阅读/预约/借书/取书），则不能使用LibraryReqCmd中的getBookId方法，否则会抛出异常`ClassCastException`。请根据相应的指令格式使用合适的方法获取书籍信息。

### 输出示例

首先 import PRINTER: `import static com.oocourse.library.LibrarySystem.PRINTER;`
...
3. 响应查询书记移动轨迹请求：`PRINTER.info(today /* 今天的日期 */, bookId /* 书籍的编号,类型为LibraryBookId */, traces/* 含有 LibraryTrace 的一个容器，包含某次移动的相关信息*/)`。
4. 响应其他请求（失败）: `PRINTER.reject(command /* 要响应的指令 */)`。
6. 响应预约、归还、还书请求（成功）: `PRINTER.accept(command /* 要响应的指令 */)`。
7. 响应还书请求（成功）(若有额外输出）: `PRINTER.accept(command /* 要响应的指令 */, additionalInfo /* 额外信息 */)`。
8. 响应其他请求（成功）: `PRINTER.accept(command /* 要响应的指令 */， bookId /* 书籍的编号,类型为LibraryBookId */)`。
...
`LibraryQcsCmd`包含一个日期（继承自`LibraryCommand`）、一个用户ID

包含如下方法：

1. `String getStudentId()` 用于获取用户ID。
2. `boolean equals(Object o)` 只比较除用户ID字段是否完全相等，不比较日期。注意两个类的实例之间不相等。
3. `int hashCode()` 将除日期外的用户ID字段进行hash。
4. `String toString()` 指令的格式化字符串。注意 LibraryQcsCmd 有方法 getCommandString() 返回不带日期的指令字符串。
