...
public class PathTest {//包含题目[3]
    ...
    public void testExtractLoopPath() {
        ...
        */
        ...
        Object newPath = path.extractLoopPath();
        if (ex && oldPath.isValid()) {
            /* normal_behavior */
            assertTrue(newPath instanceof Path);
            ArrayList<Integer> newNodes = ((Path)newPath).getNodes();
            ...
            */
            ...
            for (int i = 0; i < oldNodes.size() - 1; i++) {
                ...
                for (int j = 0; j < newNodes.size(); j++) {
                    if ((i + j) >= oldNodes.size() || !newNodes.get(j).equals(oldNodes.get(i + j))) {
                        forall = false;
                        break;
                    }
                }
            ...
            assertTrue(ex2);
            assertEquals(newNodes.get(0), newNodes.get(newNodes.size() - 1));
        }
